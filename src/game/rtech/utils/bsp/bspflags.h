...
inline std::string ContentsToString(uint32_t contents)
{
	...
	if (contents & CONTENTS_SOLID) str += "SOLID, ";
	if (contents & CONTENTS_WINDOW) str += "WINDOW, ";
	if (contents & CONTENTS_AUX) str += "AUX, ";
	if (contents & CONTENTS_GRATE) str += "GRATE, ";
	if (contents & CONTENTS_SLIME) str += "SLIME, ";
	if (contents & CONTENTS_WATER) str += "WATER, ";
	if (contents & CONTENTS_WINDOW_NOCOLLIDE) str += "WINDOW_NOCOLLIDE, ";
	if (contents & CONTENTS_OPAQUE) str += "OPAQUE, ";


	if (contents & CONTENTS_TESTFOGVOLUME) str += "TESTFOGVOLUME, ";
	if (contents & CONTENTS_PHYSICSCLIP) str += "PHYSICSCLIP, ";

	if (contents & CONTENTS_SOUNDTRIGGER) str += "SOUNDTRIGGER, ";

	if (contents & CONTENTS_NOGRAPPLE) str += "NOGRAPPLE, ";
	if (contents & CONTENTS_OCCLUDESOUND) str += "OCCLUDESOUND, ";

	if (contents & CONTENTS_IGNORE_NODRAW_OPAQUE) str += "IGNORE_NODRAW_OPAQUE, ";

	if (contents & CONTENTS_MOVEABLE) str += "MOVEABLE, ";

	if (contents & CONTENTS_TEST_SOLID_BODY_SHOT) str += "TEST_SOLID_BODY_SHOT, ";

	if (contents & CONTENTS_PLAYERCLIP) str += "PLAYERCLIP, ";
	if (contents & CONTENTS_MONSTERCLIP) str += "MONSTERCLIP, ";

	if (contents & CONTENTS_OPERATOR_FLOOR) str += "OPERATOR_FLOOR, ";
	if (contents & CONTENTS_BLOCKLOS) str += "BLOCKLOS, ";
	if (contents & CONTENTS_NOCLIMB) str += "NOCLIMB, ";
	if (contents & CONTENTS_TITANCLIP) str += "TITANCLIP, ";
	if (contents & CONTENTS_BULLETCLIP) str += "BULLETCLIP, ";
	if (contents & CONTENTS_OPERATORCLIP) str += "OPERATORCLIP, ";

	if (contents & CONTENTS_NOAIRDROP) str += "NOAIRDROP, ";

	if (contents & CONTENTS_MONSTER) str += "MONSTER, ";
	if (contents & CONTENTS_DEBRIS) str += "DEBRIS, ";
	if (contents & CONTENTS_DETAIL) str += "DETAIL, ";
	if (contents & CONTENTS_TRANSLUCENT) str += "TRANSLUCENT, ";
	if (contents & CONTENTS_BLOCK_PING) str += "BLOCK_PING, ";
	if (contents & CONTENTS_HITBOX) str += "HITBOX, ";
